...
for fold in range(folds_num):
    ...
    index_train = set(range(splits_num))
    index_test = set(range(index_test*splits_per_fold, (index_test+1)*splits_per_fold))
    index_valid = set(range(index_valid*splits_per_fold, (index_valid+1)*splits_per_fold))
    index_train = index_train - index_test - index_valid

    # select out dataset_train
    dataset_train = dataset[dataset["split"].isin(index_train)].reset_index(drop=True)
    # data augment
    if data_augment:
    ...
    with open(f"{output_pred_target_path}/valid_pred_target_fold_{fold}.pkl", "wb") as pred_y_file:
        pred_target = {"name": best_valid_pred_y["name"], "pred": [torch.tensor(best_valid_pred_y["pred"])], "target": [torch.tensor(best_valid_pred_y["target"])]}
